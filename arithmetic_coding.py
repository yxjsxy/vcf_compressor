...
class ArithmeticCoderBase(object):
	...
	def compress(self,inp,out,bitout):
		with gzip.open(out,'wb') as f_out:
			shutil.copyfileobj(inp,f_out)
...
class ArithmeticDecoder(ArithmeticCoderBase):
	...
	def decompress(self,inp,out):
		with gzip.open(inp+'/error_matrix','rb') as f_in:
			shutil.copyfileobj(f_in,out)
		data = np.loadtxt(inp+'/error.decompressed',delimiter='\t',skiprows=1)
		data = data[:,1:]
		return data
